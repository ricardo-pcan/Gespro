...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT ID_LDAP, USUARIO, PASSWORD, CONTRASENAS, FIRMADO FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( ID_LDAP, USUARIO, PASSWORD, CONTRASENAS, FIRMADO ) VALUES ( ?, ?, ?, ?, ? )";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET ID_LDAP = ?, USUARIO = ?, PASSWORD = ?, CONTRASENAS = ?, FIRMADO = ? WHERE ID_LDAP = ?";
	...
	public LdapPk insert(Ldap dto) throws LdapDaoException
	{
		...
		try {
			...
			if (dto.isContrasenasModified()) {
				if (modifiedCount>0) {
					sql.append( ", " );
					values.append( ", " );
				}
		
				sql.append( "CONTRASENAS" );
				values.append( "?" );
				modifiedCount++;
			}
		
			if (dto.isFirmadoModified()) {
				if (modifiedCount>0) {
					sql.append( ", " );
					values.append( ", " );
				}
	...
	public void update(LdapPk pk, Ldap dto) throws LdapDaoException
	{
		...
		try {
			...
			stmt.setInt( index++, pk.getIdLdap() );
			int rows = stmt.executeUpdate();
			reset(dto);
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
	...
	protected void reset(Ldap dto)
	{
		...
		dto.setContrasenasModified( false );
		dto.setFirmadoModified( false );
	}

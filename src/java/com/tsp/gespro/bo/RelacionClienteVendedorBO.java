...
public class RelacionClienteVendedorBO {
 private RelacionClienteVendedor relacionClienteVendedor = null;
	 ...
	 */
	public void deleteCliente(int idCliente) throws RelacionClienteVendedorDaoException
	{
            String SQL_DELETE = "DELETE FROM RELACION_CLIENTE_VENDEDOR WHERE ID_CLIENTE = ?";
		long t1 = System.currentTimeMillis();
		// declare variables
		final boolean isConnSupplied = (this.conn != null);
		Connection conn = null;
		...
		try {
			...
			System.out.println( "Executing " + SQL_DELETE + " with ClienteID: " + idCliente );
			stmt = conn.prepareStatement( SQL_DELETE );
			stmt.setInt( 1, idCliente );
			int rows = stmt.executeUpdate();
			long t2 = System.currentTimeMillis();
